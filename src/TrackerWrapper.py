    ...
    def TMain(self, args, seed=None, baked_server_options: Optional[Dict[str, object]] = None):
        ...
        for player in multiworld.player_ids:
            ...
            multiworld.worlds[player].options.non_local_items.value -= multiworld.worlds[
                player].options.local_items.value
...
class TrackerWrapperContext(TrackerGameContext):
    def make_gui(self):
        from kvui import GameManager
        ...
        ui = TextManager  # before the kivy imports so kvui gets loaded first
    ...
    def build_gui(self, manager: "GameManager"):
        ...
        class TrackerView(RecycleView):
            def __init__(self, **kwargs):
                super().__init__(**kwargs)
                self.data = []
                self.data.append({"text": f"Tracker Initializing for AP version {__version__}"})
